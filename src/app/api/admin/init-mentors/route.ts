...
import type { MentorDocument, UserDocument } from '@/lib/database';

// Initialize dummy mentor data for testing
  ...
  try {
    ...
    if (existingMentors > 0) {
      return NextResponse.json({ 
        message: 'Dummy mentors already exist',
        count: existingMentors 
      });
    }
    ...
    const dummyMentors: MentorDocument[] = [
      {
        _id: new ObjectId(),
    ...
    // Create corresponding user records for each mentor (optional, for authentication)
    const mentorUsers: UserDocument[] = dummyMentors.map((mentor, index) => ({
      _id: new ObjectId(),
      name: mentor.name,
      email: mentor.email,
    ...
    await usersCollection.insertMany(mentorUsers);

    return NextResponse.json({
      success: true,
      message: `Successfully created ${result.insertedCount} dummy mentors`,
      mentorIds: Object.values(result.insertedIds),
      mentors: dummyMentors.map(m => ({
        id: m._id,
        name: m.name,
        expertise: m.expertise.map(e => e.area),
        sessionTypes: m.sessionTypes.map(st => st.type),
        rating: m.rating
      }))
    });

  } catch (error) {
    console.error('Error creating dummy mentors:', {
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.error('Error fetching dummy mentors:', error);
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}

// Delete dummy mentors (for testing)
export async function DELETE(request: NextRequest) {
  try {
    ...
    // Delete dummy mentors
    const mentorResult = await mentorsCollection.deleteMany({
      email: { $in: [
        'sarah.johnson@techcorp.com',
        'michael.chen@startup.io',
        'priya.patel@designstudio.com',
    ...
    return NextResponse.json({
      success: true,
      message: `Deleted ${mentorResult.deletedCount} mentors and ${userResult.deletedCount} user records`
    });

  } catch (error) {
    console.error('Error deleting dummy mentors:', error);
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}