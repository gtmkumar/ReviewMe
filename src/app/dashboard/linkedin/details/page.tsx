...
import { 
  ...
  Star, Users, Target, Award, ChevronDown, ChevronUp, MapPin,
...
export default function LinkedInDetailsPage() {
  ...
  const loadServiceDetails = async () => {
    try {
      ...
      const historyResponse = await fetch('/api/requests?serviceType=linkedin&limit=20');
      ...
      if (cacheResponse.ok && cacheData.data.linkedin) {
        setCurrentData(cacheData.data.linkedin);
        setServiceStats(cacheData.stats.linkedin);
      }
  ...
  const getRequestDisplayName = (request: RequestHistoryItem) => {
    if (request.requestPayload?.profileUrl && request.requestPayload.profileUrl !== 'manual-entry') {
      const username = request.requestPayload.profileUrl.split('/in/')[1]?.replace('/', '') || 'Unknown';
        ...
        <div className="px-4 py-6 sm:px-0">
          ...
          <div className="mb-8">
            <div className="flex items-center space-x-4 mb-4">
              ...
              <div>
                <h1 className="text-3xl font-bold text-gray-900">LinkedIn Analysis Details</h1>
              ...
              <div className="lg:col-span-2 bg-white rounded-lg shadow p-6">
                ...
                {currentData.responseData?.profile && (
                  <div className="flex items-start space-x-6">
                    {currentData.responseData.profile.imageUrl && (
                      <img
                        src={currentData.responseData.profile.imageUrl}
                    ...
                    <div className="flex-1">
                      <h3 className="text-lg font-medium text-gray-900">
                        {currentData.responseData.profile.name}
                      </h3>
                      {currentData.responseData.profile.headline && (
                        <p className="text-linkedin text-sm">{currentData.responseData.profile.headline}</p>
                      )}
                      {currentData.responseData.profile.location && (
                        <div className="flex items-center text-sm text-gray-600 mt-1">
                          <MapPin className="h-4 w-4 mr-1" />
                          {currentData.responseData.profile.location}
                        </div>
                      )}
                      {currentData.responseData.profile.summary && (
                        <p className="text-gray-600 mt-2 text-sm line-clamp-3">{currentData.responseData.profile.summary}</p>
                      )}
                      <div className="grid grid-cols-3 gap-4 mt-4 text-sm">
                        <div className="text-center">
                          <div className="font-semibold text-gray-900">{currentData.responseData.profile.experience?.length || 0}</div>
                          <div className="text-gray-500">Experience</div>
                        </div>
                        <div className="text-center">
                          <div className="font-semibold text-gray-900">{currentData.responseData.profile.skills?.length || 0}</div>
                          <div className="text-gray-500">Skills</div>
                        </div>
                        <div className="text-center">
                          <div className="font-semibold text-gray-900">{currentData.responseData.profile.connections || 0}</div>
              ...
              <div className="bg-white rounded-lg shadow p-6">
                ...
                <div className="text-center">
                  ...
                  <div className="w-full bg-gray-200 rounded-full h-3">
                    <div 
                      className="bg-linkedin h-3 rounded-full" 
          ...
          <div className="bg-white rounded-lg shadow">
            <div className="px-6 py-4 border-b border-gray-200">
              ...
              <p className="text-sm text-gray-500 mt-1">
                Timeline of all LinkedIn analysis requests and their results
              </p>
            </div>
            ...
            <div className="divide-y divide-gray-200">
              {requestHistory.length === 0 ? (
                <div className="p-8 text-center">
                  ...
                  <p className="text-sm text-gray-500 mt-1">
                    Make your first LinkedIn analysis to see results here
                  </p>
                </div>
              ) : (
                              ...
                              <div>
                                ...
                                <div className="space-y-2 text-sm">
                                  <div className="flex justify-between">
                                    ...
                                    <span className="font-medium">{request.response.responseData.profile.name}</span>
                                  </div>
                                  {request.response.responseData.profile.headline && (
                                    <div className="flex justify-between">
                                      <span className="text-gray-600">Headline:</span>
                                      <span className="font-medium text-right max-w-48 truncate">{request.response.responseData.profile.headline}</span>
                                    </div>
                                  )}
                                  {request.response.responseData.profile.location && (
                                    <div className="flex justify-between">
                                      <span className="text-gray-600">Location:</span>
                                      <span className="font-medium">{request.response.responseData.profile.location}</span>
                                    </div>
                                  )}
                                  <div className="flex justify-between">
                                    <span className="text-gray-600">Experience:</span>
                                    <span className="font-medium">{request.response.responseData.profile.experience?.length || 0} positions</span>
                                  </div>
                                  <div className="flex justify-between">
                                    <span className="text-gray-600">Skills:</span>
                                    <span className="font-medium">{request.response.responseData.profile.skills?.length || 0} skills</span>
                                  </div>
                                  <div className="flex justify-between">
                                    <span className="text-gray-600">Connections:</span>
                                    <span className="font-medium">{request.response.responseData.profile.connections || 0}</span>
                                  </div>
                                </div>
                              </div>
                            )}
