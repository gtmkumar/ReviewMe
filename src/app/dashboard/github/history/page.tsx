...
import { 
  ...
  Calendar, CreditCard, User, Star, GitBranch, Eye, RefreshCw,
  Search, Filter, ChevronDown, BarChart3
} from 'lucide-react';
...
export default function GitHubHistoryPage() {
  ...
  const [history, setHistory] = useState<GitHubHistoryItem[]>([]);
  ...
  const loadHistory = async () => {
    try {
      setIsLoading(true);
      const response = await fetch('/api/requests?serviceType=github&limit=50');
  ...
  const getRequestDisplayName = (request: GitHubHistoryItem) => {
    if (request.requestPayload?.username) {
      return request.requestPayload.username;
    } else if (request.response?.responseData?.username) {
      return request.response.responseData.username;
    }
    return 'GitHub Analysis';
  };

  const getScoreFromRequest = (request: GitHubHistoryItem): number => {
        ...
        <div className="px-4 py-6 sm:px-0">
          ...
          <div className="mb-8">
            <div className="flex items-center space-x-4 mb-4">
              ...
              <div>
                <h1 className="text-3xl font-bold text-gray-900">GitHub Analysis History</h1>
                <p className="text-gray-600">View all your past GitHub profile analyses</p>
              </div>
            </div>
          </div>
            ...
            <div className="bg-white rounded-lg shadow p-12 text-center">
              ...
              <p className="text-gray-600 mb-6">
                {searchTerm || statusFilter !== 'all' 
                  ...
                  : 'Start by analyzing your first GitHub profile'
                }
              </p>
              <Link 
                href="/dashboard/github"
                ...
                <div key={item._id} className="bg-white rounded-lg shadow hover:shadow-md transition-shadow">
                  <div className="p-6">
                    <div className="flex items-start justify-between">
                        ...
                        <div className="flex-1">
                          <div className="flex items-center space-x-3 mb-2">
                            ...
                            {item.status === 'completed' && item.response?.analysisResults?.score && (
                              <div className="flex items-center space-x-1 text-github">
                            ...
                            <div className="bg-gray-50 rounded-lg p-4">
                              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                ...
                                {item.response.responseData && (
                                  <div>
                                    <h4 className="text-sm font-medium text-gray-700 mb-1">Profile Stats</h4>
                                    <div className="text-sm text-gray-600 space-y-1">
                                      {item.response.responseData.profile?.public_repos && (
                                        <div className="flex items-center space-x-1">
                                          <GitBranch className="h-3 w-3" />
                                          <span>{item.response.responseData.profile.public_repos} repos</span>
                                        </div>
                                      )}
                                      {item.response.responseData.profile?.followers && (
                                        <div className="flex items-center space-x-1">
                                          <User className="h-3 w-3" />
                                          <span>{item.response.responseData.profile.followers} followers</span>
                                        </div>
                                      )}
                                    </div>
                                  </div>
                      ...
                      <div className="flex items-center space-x-2">
                        <Link
                          href={`/dashboard/github/details?requestId=${item._id}`}
